...
## [0.5.1] - Thu Jul 09 20:02:56 JST 2026

### Changed
- **文字サイズをサンプル（実物の反転フラップ式表示機）に近づけて拡大**: Description本文 0.875rem→1.3rem、WHOバッジ 0.65rem→1.05rem、数字フラップ 0.85rem→1.4rem（幅・高さも比例して拡大）、行の高さ 3.25rem→4.25rem
- **WHOバッジをセル全面のフラップに変更**: これまで文字サイズに合わせた小さいピル表示だったが、`.flap-cell-who`をセルいっぱいに引き伸ばし（align-items: stretch）、バッジ自体も width:100%/height:100% にすることで、セル全体が1枚の面として回転して見えるように修正
- **タブ/スコープ切替時に上の行から順にめくれるように変更**: `SplitFlapBoard.tsx`に`useStaggeredItems`フックを新設。切替の瞬間に全行が一斉に反映されていたのを、1行ずつ55ms間隔で段階的に反映する方式に変更。各行はそれまでの表示を保ったまま新しい値への差分だけをめくるため、実機の掲示板のように上から下へ切り替わりが伝播していく見た目になった
